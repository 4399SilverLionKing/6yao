...
export function HexagramCard({ label, hexagram }: HexagramCardProps) {
  return (
    <article className="border-ink/10 rounded-3xl border bg-white/60 p-5">
      ...
      <h2 className="mt-3 text-2xl font-semibold">{hexagram.fullName}</h2>
      <p className="text-ink/65 mt-2 text-sm">第 {hexagram.sequence} 卦 · {hexagram.name}</p>
      ...
      <p className="text-ink/80 mt-3 text-sm">卦义：{hexagram.meaning}</p>
    </article>
  );
}
